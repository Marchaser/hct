function [SimulateResult, ExitFlag] = SIMULATE_TRANS(VfiTransResult, SimulateSsResult, Params, Rt, Wt, Trt, Tsst)
...
SimulateResult.kp = SimulateResult.k;
SimulateResult.hp = SimulateResult.h;
...
% SimulateResult.vFull = zeros(NumOfAgents,J,J);
% SimulateResult.kFull = zeros(NumOfAgents,J,J);
% SimulateResult.hFull = zeros(NumOfAgents,J,J);
...
for t=1:TTrans
    SimulateResult.k = SimulateResult.kp;
    SimulateResult.h = SimulateResult.hp;
    switch(ControlSpec)
        case {'Base','Exn'}
        ...
        case 'Exhc'
            % (control,A,K,KBar,HBar,Age)
            PolicyPp = tensor_pchip({KGrid,KBarGrid,HBarGrid}, reshape(permute(squeeze(PolicyYoung(:,:,:,:,:,:,t)), [1 6 2 3 4 5]), [], KPts, KBarPts, HBarPts));
            PolicyVecSize = [2 Jw APts];
    end
    PolicyPp = myppual(PolicyPp);
...
%         PolicyInterpIdx = sub2ind(PolicyOldVecSize, (j-Jw)*ones(1, NumOfAgents),t*ones(1,NumOfAgents));
        ...
        SimulateResult.kp(:,j+1) = X;
        
        SimulateResult.c(:,j) = (Budget - SimulateResult.kp(:,j+1)) / (1+TauC);
    ...
    for j=Jw:-1:1
        ...
        % next period h
        SimulateResult.hp(:,j+1) = EpsilonShock(:,j) .* ...
    ...
    if (size(SimulateResult.k,2)==J+1)
        SimulateResult.k(:,end) = [];
    end
    if (size(SimulateResult.population,2)==J+1)
...
SimulateResult = v2struct(C,TaxC,K,L,Tss,TaxInc,Tax,G,Tr,Y,GYRatio,KYRatio,KLRatio,R,W,MeanHours);
ExitFlag = 0;
end
