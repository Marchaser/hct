...
%{
Note(wenlan): Here's why I can't do a full construction and reduction to
ppK, and pass it to optimizer in simulation
Because (KBar, HBar) are individual psuedos states, if one constructs a
spline w.r.t. K for every single agent, the memory usage is very high.
This is different from problems with macro psudo states (e.g. KS)
%}
...
for j=1:1:Jw
  ...
  ProblemYoung.x0 = X;
  [X, V, OptExitFlag] = dpopt_utility_exhc_simu(ProblemYoung);
