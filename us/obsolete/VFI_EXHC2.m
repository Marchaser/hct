...
for j = Jw:-1:1
  ...
  V0Pp = myppual(V0Pp);
  %}
  % Do a construction w.r.t. KBar and HBar, treating KGrid as vector functions
  ...
  KBarPrime = reshape(KBarPrime, [EpsilonPts KPts APts KBarPts HBarPts]);
  ...
  HBarPrimeBeforeShock = reshape(HBarPrimeBeforeShock, [EpsilonPts, KPts APts KBarPts HBarPts]);
  % Expand to accomodate Epsilon shock
  HBarPrime = HBarPrimeBeforeShock .* repmat(EpsilonGrid(:), [1 KPts APts KBarPts HBarPts]);
  
  for iA = 1:APts
      V0KInterpIdx = sub2ind([APts KPts], iA*ones(1,KPts), 1:KPts);
      % Expand to accomodate Epsilon shock
      V0KInterpIdx = repmat(V0KInterpIdx, EpsilonPts, 1);
      V0KInterpIdx = V0KInterpIdx(:)';
      for iHBar = 1:HBarPts
          for iKBar = 1:KBarPts
              V0K = myppual(V0PpAtKBarHBarPp, ...
                  [reshape(KBarPrime(:,:,iA,iKBar,iHBar),1,[]);reshape(HBarPrime(:,:,iA,iKBar,iHBar),1,[])], ...
                  [], V0KInterpIdx);
              % Integrate over Epsilon
              EV0K = EpsilonP * reshape(V0K, EpsilonPts, KPts);
              % Construct w.r.t. K
              V0KPp = tensor_pchip(KGrid, EV0K);
              % Solve the opt problem
              ProblemYoung.pp = myppual(V0KPp);
              ProblemYoung.Data = [...
                  KGrid(:)';
                  SBar(iA,iKBar,iHBar)*ones(1,KPts);
                  HBarMesh(iA,iKBar,iHBar)*ones(1,KPts);
                  KInc(:)';
                  ];
              % Note(wenlan): use last iteration solution as initial guess for this
              % iteration
              ProblemYoung.x0 = reshape(X(:, iA, :, iKBar, iHBar), [2 KPts]);
              if (any(ProblemYoung(:).lb > ProblemYoung(:).ub))
                  VfiResult = [];
                  ExitFlag = -3;
                  return;
              end
              ProblemYoung.idx = ones(1, KPts);
              [XSub, VSub, OptExitFlag] = dpopt_utility_young_exhc2(ProblemYoung);
              X(:,iA,:,iKBar,iHBar) = reshape(XSub, [2 1 KPts 1 1]);
              V(iA,:,iKBar,iHBar) = reshape(VSub, [1 KPts 1 1]);
          end
      end
  end
