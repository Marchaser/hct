function [SimulateResult, ExitFlag] = SIMULATE_FACTOR_TAX(VfiResult, Params)
...
% simulate young
% NOTE(wenlan): Construct value function spline. Keep in mind, vec has the memory order (age, a)
%{
...
for j=Jw+1:1:J
  ...
  if (any(ProblemOld(:).lb > ProblemOld(:).ub))
      SimulateResult = [];
      ExitFlag = -3;
      return;
  end
