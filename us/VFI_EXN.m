function [VfiResult, ExitFlag] = VFI_EXN(Params)
...
for j = Jw:-1:1
  ...
  % update policy and value
  PolicyYoung(:,:,:,:,:,j) = reshape(X, [2 APts KPts HPts KBarPts HBarPts]);
  ValueYoung(:,:,:,:,j) = reshape(V, [APts KPts HPts KBarPts HBarPts]) * Beta * SurvivalRate(j);
end
